...
namespace _01.Carlos.Fontes.Controllers
{
    ...
    [Authorize]
    public class AlterarPrecoCachecol : Controller
    {
        private readonly ApplicationDbContext db;

        // Construtor que procede à injeção da instância do contexto da base de dados
        public AlterarPrecoCachecol(ApplicationDbContext context)
        {
            db = context;
        }
        ...
        public IActionResult Index(string novoPreco, string clicado)
        {
            ...
            {
                // Filtra o conjunto de registos na entidade Produtos que correspondem à denominação "Cachecol Oficial"
                var produto = db.Produtos.Where(p => p.Nome == "Cachecol Oficial");
            ...
            ViewBag.precoAtualCachecol = db.Produtos.Where(p => p.Nome == "Cachecol Oficial").Select(p => p.Preco).FirstOrDefault();
