...
namespace _01.Carlos.Fontes.Controllers
{
    ...
    [Authorize]
    public class AlterarPrecoCamisolaCasa : Controller
    {
        private readonly ApplicationDbContext db;

        // Construtor que procede à injeção da instância do contexto da base de dados
        public AlterarPrecoCamisolaCasa(ApplicationDbContext context)
        {
            db = context;
        }
        ...
        public IActionResult Index(string novoPreco, string clicado)
        {
            ...
            {
                // Filtra o conjunto de registos na entidade Produtos que correspondem à denominação "Camisola Casa SCVR"
                var produto = db.Produtos.Where(p => p.Nome == "Camisola Casa SCVR");
            ...
            ViewBag.precoAtualCamisolaCasa = db.Produtos
                .Where(p => p.Nome == "Camisola Casa SCVR")
                .Select(p => p.Preco)
                .FirstOrDefault();
